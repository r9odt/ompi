...
int mca_coll_sm_scatter_intra(const void *send_buff, const int send_count,
   ...
   *********************************************************************/
    ...
    while (max_bytes < total_size) {
      ...
      FLAG_SETUP(flag_num, flag, data);
      FLAG_WAIT_FOR_OP(flag, data->mcb_operation_count, scatter_nonroot_label1);
      ...
      do {
        ...
        WAIT_FOR_NOTIFY(comm_rank, index, max_data, scatter_nonroot_label2);
