...
/**
 * @brief shared-memory based algorithm for Gather using XPMEM ZeroCopy.
                       ...
                       mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT_VERBOSE((SHARM_LOG_FUNCTION_INFO, mca_coll_sharm_stream,
                         "coll:sharm:%d:gather_xpmem: (%d/%d/%s) root %d",
    ...
    coll_info->rbuf[comm_rank][0] = (ptrdiff_t) rbuf;
    ...
    for (int i = 0; i < comm_size; ++i) {
        coll_info->rcounts[comm_rank][i] = rdtype_size * rcount;
    }
    ...
    if (0 == is_contiguous_dtype) {
        opal_output_verbose(SHARM_LOG_ALWAYS, mca_coll_sharm_stream,
                            "coll:sharm:%d:gather_xpmem: (%d/%d/%s) "
    ...
    if (root == comm_rank && MPI_IN_PLACE != sbuf) {
        char *rbuf_offset = (char *) rbuf + rext * rcount * comm_rank;
        ...
        char *peer_buff = (char *) coll_info->rbuf[root][0]
                          + total_size * comm_rank;
        ...
        size_t aligned_total_size = (total_size + align_offset
                                     + shm_data->mu_page_size - 1)
                                    & ~(shm_data->mu_page_size - 1);
        SHARM_PROFILING_TIME_START(sharm_module, gather, xpmem_attach);
        apid = xpmem_get(segid, XPMEM_RDWR, XPMEM_PERMIT_MODE, NULL);
        if (apid < 0) {
            opal_output_verbose(
                SHARM_LOG_ALWAYS, mca_coll_sharm_stream,
                "coll:sharm:%d:gather_xpmem: (%d/%d/%s) can not get apid of"
        ...
        if (xpmem_seg_addr < -1) {
            opal_output_verbose(
                SHARM_LOG_ALWAYS, mca_coll_sharm_stream,
                "coll:sharm:%d:gather_xpmem: (%d/%d/%s) can not attach of"
                "shared memory region, error code %ld",
                SHARM_COLL(gather, sharm_module), comm_rank,
        ...
        SHARM_PROFILING_TIME_STOP(sharm_module, gather, xpmem_attach);

        /* Copy to root process by non-roots */

        SHARM_PROFILING_TIME_START(sharm_module, gather, copy);
        memcpy((void *) (xpmem_seg_addr + align_offset), _sbuf, total_size);
        SHARM_PROFILING_TIME_STOP(sharm_module, gather, copy);
    ...
    OPAL_OUTPUT_VERBOSE(
        (SHARM_LOG_FUNCTION_INFO, mca_coll_sharm_stream,
         "coll:sharm:%d:gather_xpmem: (%d/%d/%s), root %d gather complete",
