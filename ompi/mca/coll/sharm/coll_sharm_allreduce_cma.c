...
/**
 * @brief shared-memory based algorithm for Allreduce using CMA approach.
                        ...
                        mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT_VERBOSE((SHARM_LOG_FUNCTION_INFO, mca_coll_sharm_stream,
                         "coll:sharm:%d:allreduce_cma: (%d/%d/%s)",
    ...
    *(ptrdiff_t *) (coll_info) = (ptrdiff_t) _sbuf;
    coll_info = coll_info + sizeof(ptrdiff_t);
    *(ptrdiff_t *) (coll_info) = (ptrdiff_t) sbuf;
    ...
    SHARM_PROFILING_TIME_START(sharm_module, allreduce, collective_exchange);
    ...
    while (
        ...
        for (int i = 0; i < comm_size; ++i) {
            ...
            SHARM_PROFILING_TIME_START(sharm_module, allreduce, pop);
            int pop = sharm_queue_pop_contiguous(
                RESOLVE_COLLECTIVIES_DATA(sharm_module, i)
                    + collectivies_info_bytes_received_by_rank[i],
                i, comm, sharm_module);
            SHARM_PROFILING_TIME_STOP(sharm_module, allreduce, pop);
        ...
        SHARM_PROFILING_TIME_START(sharm_module, allreduce, push);
        ...
        SHARM_PROFILING_TIME_STOP(sharm_module, allreduce, push);
        collectivies_info_bytes_sended += push;
    }

    uint8_t is_contiguous_dtype = 1;
    char has_mpi_in_place = 0;
    for (int i = 0; i < comm_size; ++i) {
        char *coll_info_is_contigous = RESOLVE_COLLECTIVIES_DATA(sharm_module,
                                                                 i);
        is_contiguous_dtype &= *coll_info_is_contigous;
        ptrdiff_t *original_sbuf = (ptrdiff_t *) (coll_info_is_contigous
                                                  + sizeof(char)
                                                  + sizeof(ptrdiff_t));
        has_mpi_in_place |= (original_sbuf == MPI_IN_PLACE);
    }

    SHARM_PROFILING_TIME_STOP(sharm_module, allreduce, collective_exchange);
    ...
    if (OPAL_UNLIKELY(1 == has_mpi_in_place)) {
        opal_output_verbose(SHARM_LOG_ALWAYS, mca_coll_sharm_stream,
                            "coll:sharm:%d:allreduce_cma: (%d/%d/%s) "
                            "Unsupported MPI_IN_PLACE buffers",
                            SHARM_COLL(allreduce, sharm_module), comm_rank,
                            comm_size, comm->c_name);
        return OMPI_ERR_NOT_SUPPORTED;
    }
    ...
    OPAL_OUTPUT_VERBOSE(
        (SHARM_LOG_FUNCTION_INFO, mca_coll_sharm_stream,
         "coll:sharm:%d:allreduce_cma: (%d/%d/%s), allreduce complete",
         SHARM_COLL(allreduce, sharm_module), comm_rank, comm_size,
         comm->c_name));
